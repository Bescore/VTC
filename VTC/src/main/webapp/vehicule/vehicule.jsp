...
<c:choose>
		...
		<tbody>
			<c:forEach items="${listOfVehicule}" var="vehi">
				<tr>
					...
					<td class="modif"><a href="/VTC/vehicule?id=${vehi.id_vehicule}">	
								<i class="fa-solid fa-feather-pointed"></i></a></td>
					<td><button type="submit" id="delete" name="submit" class="btn btn-primary" value="delete-${vehi.id_vehicule }"><i class="fa-solid fa-trash-can"></i></button></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<c:if test="${empty marque}">
		...
		<div class="form-group">
			<label for="exampleInputEmail1">Marque</label> <input type="text"
				class="form-control" id="marque" name="marque"
				aria-describedby="emailHelp" placeholder="marque" value="${marque }">
		</div>
		</c:if>
		<br>
		...
		<div class="form-group">
			<label for="exampleInputPassword1">Modele</label> <input type="text"
				class="form-control" id="modele" name="modele" placeholder="modele" value="${modele }">
		</div>
		</c:if>
		<br>
		<c:if test="${empty couleur}">
		<div class="form-group">
			<label for="exampleInputPassword1">couleur</label> <input type="text"
				class="form-control" id="couleur" name="couleur"
		...
		<div class="form-group">
			<label for="exampleInputPassword1">immatriculation</label> <input
				type="text" class="form-control" id="immatriculation" name="immatriculation"
		...
		<div class="form-group">
			<label for="exampleInputPassword1">immatriculation</label> <input
				type="text" class="form-control" id="immatriculation" name="immatriculation"
				placeholder="immatriculation" value="${couleur }">
		</div>
		</c:if>
		<br>
		<button type="submit" id="submit_assoc" name="submit" class="btn btn-primary" value="add">Ajouter
			cette association</button>
			<c:if test="${!empty couleur}">
		<button type="submit" id="modif_conducteur" name="submit" class="btn btn-primary" value="update-${id_vehi }">Modifier</button>
		</c:if>
	</form>
</div>
</body>
